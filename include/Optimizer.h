...
private:
	...
	double m0;
    const int L, L2, ranks, nprocs;
public:
	fcost(const EQUATION &Equation, const std::vector<double> dilute, const double m0, const int ranks, const int nprocs) : 
    equation(Equation),
    dilute(dilute), m0(m0),
    L(equation.get_L()),
    L2(L*2),
	...
	{	
		...
		for(int i=0;i<L;i++){
			...
			dd += g[i]/(double)L; dm += g[i+L]/(double)L;
		}
		if(ranks==0) std::cout<<"   dm : "<<dm<<" , dd : "<<dd<<std::endl;
	}
};
...
class Minimizer_fixedNum 
{
    ...
    int minimizer(std::vector<double> &m, std::vector<double> &d, const std::vector<double> &dilUsite, double &Num, const double &m0, const int ranks, const int nprocs)
    {
        ...
        FCOST constrained_FreeEnergy(equation,dilUsite,m0,ranks,nprocs);
