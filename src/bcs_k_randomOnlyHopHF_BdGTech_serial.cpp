...
int main(int argc, char* argv[])
{
    ...
    options.push_back(pair_t("Tag", "tag number of disorder samples"));
    ...
    std::string filename = parser.find<>("path") + "/KagomeProfile-L" + std::to_string(L) + "U" + parser.find<>("U") + "X" + parser.find<>("X") + "N" + std::to_string(N_tot) + "-TAG" + parser.find<>("Tag") + ".dat"; 
#ifdef __OUTFILE__
    ...
    std::cout << "\n# duration time : " << (clock()-start)/(double)CLOCKS_PER_SEC << std::endl;
#ifdef __OUTFILE__
    for (int j=0;j<L;++j)
    {
        int supX = (j/3)%l;
