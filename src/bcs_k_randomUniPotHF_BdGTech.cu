...
int main(int argc, char* argv[])
{
    ...
    double n,mg,mi_,di_;
    std::vector<double> m(L), d(L), dilUsite(L,U);
    ...
    for (int i=0; i<Nsam; ++i)
    {
        ...
        std::string filename = parser.find<>("path") + "/KagomeProfile-L" + std::to_string(L) + "U" + parser.find<>("U") + "X" + parser.find<>("X") + "N" + std::to_string(N_tot) + "-TAG" + std::to_string(i) + ".dat"; 
#ifdef __OUTFILE__
