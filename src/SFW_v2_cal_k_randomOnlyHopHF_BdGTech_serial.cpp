...
int main(int argc, char* argv[])
{
	...
	std::string filename, lfilename, N_tot_str=std::to_string((int)N_tot); //remove_zeros_in_str(N_tot);
    ...
    filename = parser.find<>("path") + "/RandomOnlyHopKagome_SFW_v2-L" + std::to_string(L) + "U" + parser.find<>("U") + "X" + parser.find<>("X") + "N" + N_tot_str + "-TAG" + parser.find<>("Tag") + ".dat"; 
#ifdef __OUTFILE__
